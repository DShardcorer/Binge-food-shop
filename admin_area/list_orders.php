...
<table class="table table-bordered mt-5">
    ...
    <tbody class='bg-secondary text-light'>
        <?php
        $get_order = "SELECT * FROM user_orders JOIN user_table ON user_orders.user_id = user_table.user_id";

        // Filter by order status
        if (isset($_GET['order_status']) && !empty($_GET['order_status'])) {
            $order_status_filter = pg_escape_string($con, $_GET['order_status']);
            $get_order .= " WHERE order_status = '$order_status_filter'";
        }
        ...
        if (
            ...
            // Assuming your date column in the database is named 'order_date'
            $get_order .= " AND order_date BETWEEN '$start_date' AND '$end_date'";
        }
        
        $get_order .= " ORDER BY order_id DESC";
