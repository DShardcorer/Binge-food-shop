...
ALTER TABLE products ADD COLUMN amount_sold int NOT NULL DEFAULT 0;
--function to update amount_sold with product id as input
CREATE OR REPLACE FUNCTION update_amount_sold(product_id_input int, amount_sold_input int)
RETURNS void AS
$$
BEGIN
    UPDATE products
    SET amount_sold = amount_sold_input
    WHERE prod_id = product_id_input;
END;
$$
LANGUAGE plpgsql;
--call function
SELECT update_amount_sold(1, 10);
